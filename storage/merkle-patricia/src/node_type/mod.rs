...
impl NodeKey {
    ...
    pub fn new(hash: HashValue, nibble_path: NibblePath) -> Self {
        Self { hash, nibble_path }
    }

    /// A shortcut to generate a node key consisting of a version and an empty nibble path.
    pub fn new_empty_path(hash: HashValue) -> Self {
        Self::new(hash, NibblePath::new(vec![]))
    }
    ...
    pub fn gen_child_node_key(&self, hash: HashValue, n: Nibble) -> Self {
        let mut node_nibble_path = self.nibble_path().clone();
        node_nibble_path.push(n);
        Self::new(hash, node_nibble_path)
    }
    ...
    pub fn gen_parent_node_key(&self) -> Self {
        ...
        Self::new(self.hash, node_nibble_path)
    }

    /// Sets the hash to the given hash.
    ...
    pub fn decode(val: &[u8]) -> Result<NodeKey> {
        ...
        Ok(NodeKey::new(hash, nibble_path))
    }
}
