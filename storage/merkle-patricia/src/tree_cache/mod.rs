...
pub struct TreeCache<'a, R: 'a + TreeReader> {
    ...
    /// The last hash of the transaction to which the upcoming `put`s will be related.
    current_last_hash: HashValue,
    /// Intermediate nodes keyed by node hash
    node_cache: HashMap<NodeKey, Node>,

    parent_node: HashMap<NodeKey, NodeKey>,
    ...
    R: 'a + TreeReader,
{
    ...
    pub fn new(reader: &'a R, current_last_hash: HashValue, next_version: Version) -> Self {
    ...
    /// Put node parent relationship
    pub fn put_parent_key(&mut self, node_key: NodeKey, parent_key: NodeKey) -> Result<()> {
