...
impl ChildInfo {
    ...
    fn into_child(self) -> Child {
        match self {
            ...
            Self::Leaf { node } => {
                Child::new(node.hash(), true /* is_leaf */)
            }
        }
    }
}
...
impl InternalInfo {
    ...
    /// fully initialized.
    fn into_internal_node(mut self) -> (NodeKey, InternalNode) {
        ...
        for (index, child_info_option) in self.children.iter_mut().enumerate() {
            if let Some(child_info) = child_info_option.take() {
                children.insert((index as u8).into(), child_info.into_child());
            }
        }
...
{
    pub fn new(store: &'a S, expected_root_hash: HashValue) -> Result<Self> {
        let (partial_nodes, previous_leaf) = match store.get_rightmost_leaf()? {
            Some((node_key, leaf_node)) => {
                ...
                (
                    Self::recover_partial_nodes(store, node_key)?,
                    Some(leaf_node),
                )
            }
            None => {
                ...
                (
                    vec![InternalInfo::new_empty(NodeKey::new_empty_path(
                        HashValue::zero(),
                    ))],
                    None,
                )
    ...
    fn recover_partial_nodes(
        ...
        loop {
            ...
            for i in 0..previous_child_index.unwrap_or(16) {
                //TODO is zero?
                let child_node_key =
                    node_key.gen_child_node_key(HashValue::zero(), (i as u8).into());
    ...
    fn insert_at_leaf<'b>(
        ...
        for _ in num_existing_partial_nodes..common_prefix_len {
            ...
            let new_node_key = NodeKey::new(HashValue::zero(), visited_nibbles);
        ...
        match last_node.children[rightmost_child_index] {
            Some(ChildInfo::Leaf { ref node }) => {
                let child_node_key = last_node
                    .node_key
                    //TODO is expected_root_hash?
                    .gen_child_node_key(
                        self.expected_root_hash,
                        (rightmost_child_index as u8).into(),
                    );
                self.frozen_nodes
    ...
    fn freeze_internal_nodes(&mut self, num_remaining_nodes: usize) {
        while self.partial_nodes.len() > num_remaining_nodes {
            ...
            let (node_key, internal_node) = last_node.into_internal_node();
